...
设置商业区块等）自 2026-09-09 起移出本仓，由 nuwa-work 经 overlay 文件覆写注入
...
| `NUWAX_UPDATE_FEED_BASE` | `...aliyuncs.com/nuwaclaw-electron` | `...aliyuncs.com/nuwa-work-electron` |
...
| `nuwax/` | （已移除）前端 pin 归产品壳——如 nuwa-work 根 `nuwax/` 子模块；基座 dev 经 `NUWAX_FRONTEND_DIST` env 接 dist |
...
- **推荐独立工作副本开发**（非经产品壳的 submodule）：`git clone
  ...
  设 `NUWAX_FRONTEND_DIST=<dist 目录>` 并同步 nuwa-work overlay 后运行）。
